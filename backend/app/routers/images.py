...
def list_images_for_set(
    ...
    if not img_set:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="ImageSet not found"
        )
...
def render_image(
    ...
    if not file_path.exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="DICOM file not found on disk"
        )

    try:
        png_bytes = render_dicom_as_png(
            file_path, center=float(center), width=float(width)
        )
