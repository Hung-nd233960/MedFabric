...
def undo_doctor_columns(df: pd.DataFrame, doctor_id: str) -> pd.DataFrame:
    ...
    text_target_cols = [
        col for col in df.columns if f"_{doctor_id}" in col and "image" in col
    ]
    numeric_target_cols = [
        col
        for col in df.columns
        if f"_{doctor_id}" in col and col not in text_target_cols
    ]
    ...
    any_not_na = (
        df[numeric_target_cols].notna().any(axis=1)
        if numeric_target_cols
        else pd.Series(False, index=df.index)
    )
    any_not_empty_str = (
        df[text_target_cols].astype(str).ne("").any(axis=1)
        if text_target_cols
        else pd.Series(False, index=df.index)
    )
