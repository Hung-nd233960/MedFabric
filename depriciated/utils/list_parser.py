...
if __name__ == "__main__":
    ...
    class TestListParser(unittest.TestCase):
        ...
        def test_get_annotation_error(self):
            self.assertEqual(
                get_annotation_error("1,2,2", 10), AnnotateError.NOT_UNIQUE
            )
            self.assertEqual(
                get_annotation_error("5-3", 10), AnnotateError.INVALID_RANGE
            )
            self.assertEqual(
                get_annotation_error("a,b,c", 10), AnnotateError.INVALID_SYNTAX
            )
            self.assertEqual(
                get_annotation_error("1,4,3", 10), AnnotateError.NOT_ORDERED
            )
            self.assertEqual(
                get_annotation_error("1,2,11", 10), AnnotateError.OUT_OF_BOUNDS
            )
