...
from depriciated.app_state import AppState, Page, can_transition
...
def render_evaluation_controls(app: AppState, num_images: int) -> None:
    ...
    def render_annotation_section(
        label: str, key_img: str, key_score: str, error_field: str, max_value: int
    ):
        img_input = st.text_input(
            f"{label} Images:", key=key_img, max_chars=100, placeholder=placeholder_text
        ).strip()
        ...
        st.number_input(
            f"{label} Score:", min_value=0, max_value=max_value, step=1, key=key_score
        )

    # UI: text + number inputs
    col3, col4 = st.columns(2)
    with col3:
        render_annotation_section("Basel", key_basel_img, key_basel_score, "basel", 4)
    with col4:
        render_annotation_section(
            "Corona", key_corona_img, key_corona_score, "corona", 6
        )
    ...
    print(
        f"{app.current_set.patient_id} - Basel: {app.current_set.basel_image}, Corona: {app.current_set.corona_image}"
    )
...
def render_annotation_errors(error: AnnotateError) -> None:
    ...
    if error == AnnotateError.INVALID_SYNTAX:
        st.error(
            "Invalid syntax! Please use a comma-separated list of numbers or ranges (e.g., 1,3,5-7)."
        )
...
def render_navigation_controls(app: AppState, mode: AnnotateMode) -> None:
    ...
    with ccol1:
        ...
        if has_any_errors():
            st.warning(
                "⚠️ Some annotations are invalid. Please correct them before confirming."
            )
        else:
